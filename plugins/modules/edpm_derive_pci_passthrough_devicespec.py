...
DOCUMENTATION = '''
---
module: edpm_derive_pci_passthrough_devicespec
author:
    - OpenStack EDPM Contributors
version_added: '1.0'
short_description: Derive list of pci device_spec for sriov nic-partition
notes: []
description:
  - This module generates the pci device_spec addr list for nova configmap,
    when sriov nic partitioning is configured in EDPM nodes.
    See openstack user guide for details of PCI passthrough formats.
options:
...
PASSTHROUGH_DEVSPEC_KEY = 'device_spec'
...
def get_pci_regex_pattern(config_regex, size, maxval, hex_value):
    """
    Get the the pci regex based on user value
    """
...
def get_user_regex_from_addrdict(addr_dict):
    """
    Get the the pci regex based on user dict
    """
...
def get_pci_addr_from_ifname(ifname):
    ...
    """
    dev_path = os.path.join(SYS_CLASS_NET_PATH, ifname, "device")
...
def get_sriov_configs():
    """
    Read nic partitioned sriov host config
    """
    configs = []
    try:
...
def get_allocated_pci_addresses(configs):
    """ Get host allocated PCI addresses
    """
    alloc_pci_info = []
    for config in configs:
...
def get_pci_passthrough_devicespec(user_config, pf, pci_addresses):
    """ Replace devname in user_config with pci addresses
    """
...
def generate_combined_configuration(user_configs, system_configs,
    ...
    for user_config in user_configs:
        if 'address' in user_config and 'devname' in user_config:
            msg = f"Both devname and address can't be present in {PASSTHROUGH_DEVSPEC_KEY}"
            raise InvalidConfigException(msg)
        ...
        if not any(k in user_config for k in keys):
            ...
            pf = get_pf_name_from_phy_network(user_config['physical_network'], phys_dev_map)
...
def _run_derive_pci(user_configs,
    ...
    if user_config_copy != user_configs:
        ...
        with open(sriov_nova_conf_file, 'w') as pci_file:
            json.dump(pci_passthrough, pci_file)
        changed = True
    else:
        print("NOTE: user_configs is good, retaining same for nova confgmap")
        pci_passthrough['device_spec'] = user_configs
        with open(sriov_nova_conf_file, 'w') as pci_file:
            json.dump(pci_passthrough, pci_file)
    return changed, pci_passthrough


def main():
    derived = {}
    changed = False
    result = dict(success=False, message="", changed=False, device_spec={})
    ...
    # Set parameters
    sriov_device_spec = module.params.get('sriov_device_spec_map', None)
    phydev_map = module.params.get('sriov_phydev_map', None)
    nova_conf = module.params.get('sriov_device_spec_out_file', None)

    if len(sriov_device_spec) == 0:
        ...
        changed, derived = _run_derive_pci(sriov_device_spec, nova_conf, phydev_map)

    # The user devspec updated / or not is valid
    if changed:
        result['message'] = "PCI device_spec is generated"
    else:
        result['message'] = "No change in device_spec or input error"
    result['device_spec'] = derived
    result['rc'] = 0
    result['changed'] = True
    result['success'] = True
