...
DOCUMENTATION = '''
    strategy: edpm_free
    short_description: EDPM specific free strategy
    description:
...
class EdpmFreeBreak(Exception):
    """Exception used to break loops"""
    pass


class EdpmFreeContinue(Exception):
    ...
    def _advance_host(self, host, task):
        ...
        if run_once:
            if action and getattr(action, 'BYPASS_HOST_LOOP', False):
                ...
                display.warning("Using run_once with the edpm_free strategy is not currently supported. "
        ...
        while True:
            ...
            if host_name not in self._tqm._unreachable_hosts and t:
                ...
                if not self._blocked_hosts.get(host_name, False):
                    ...
                    except EdpmFreeBreak:
                        break
                    except EdpmFreeContinue:
                        continue
                else:
