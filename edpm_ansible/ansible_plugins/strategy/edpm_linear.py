...
try:
    ...
    BASESPEC = importlib.util.spec_from_file_location(
        'edpm_base',
        os.path.join(os.path.dirname(__file__), 'edpm_base.py')
    )
...
DOCUMENTATION = '''
    strategy: edpm_linear
    short_description: EDPM specific linear strategy
    description:
...
class EdpmLinearTerminated(Exception):
    """Exception for terminated state"""
    pass


class EdpmLinearNoHostTask(Exception):
    """Exception for no host task"""
    pass


class EdpmLinearRunOnce(Exception):
    """Exception for run once"""
    pass


class StrategyModule(BASE.EdpmBase):
    ...
    def process_work(self):
        ...
        for (host, task) in host_tasks:
            ...
            except EdpmLinearNoHostTask:
                continue
            except (EdpmLinearTerminated, EdpmLinearRunOnce):
                break
