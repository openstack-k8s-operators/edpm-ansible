...
for INFO in ${CEPH_INFO[@]}; do
    ...
    # Ensure the secret XML file exists.
    # If the edpm_nova_libvirt role is used standalone without puppet, then
    # secret.xml won't exist. Create the first cluster's secret with this script.
    if [ ! -f "/etc/nova/secret.xml" ]; then
        HAVE_FIRST_SECRET=0
        SECRET_FILE="/etc/nova/secret.xml"
        SECRET_NAME="client.${CLIENT} secret"
    else
        HAVE_FIRST_SECRET=1 # either puppet or this script created it
    fi
    # If puppet, or this script created, a secret.xml file for the
    # first cluster's secret, detect when to use that file.
    if [ $HAVE_FIRST_SECRET -eq 1 ]; then
