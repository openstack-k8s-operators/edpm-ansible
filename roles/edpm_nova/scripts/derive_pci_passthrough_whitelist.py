...
_DERIVED_PCI_WHITELIST_FILE = '/var/lib/openstack/pci_passthrough_whitelist.json'
...
# Todo: (remove)This function will not be needed now
def get_neutron_devmap_configs():
    ml2_sriov = []
    valid_spec = 'physical_device_mappings'

    # Read phydev mapping from neutron conf file
    try:
        with open('/var/lib/os-net-config/01-neutron-sriov-agent.conf') as neutron_config:
            for line in neutron_config:
                line = line.replace('\n', '')
                if line.startswith(valid_spec):
                    phy_map = re.split(', |,|=|= ', line)
                    for i in phy_map:
                        if ':' in i:
                            ml2_sriov.append(i)
    except IOError:
...
def get_pf_name_from_phy_network(physical_network):
    out = get_neutron_devmap_configs()
    if len(out):
...
def get_nova_userconfigs(nova_conf_file):
    configs = []
    src_config = ""
    # Currently supporting both formats
    valid_spec = ['passthrough_whitelist', 'device_spec']
    ...
    try:
        with open(nova_conf_file) as nova_config:
            for line in nova_config:
                pf = line.strip()
                if pf.startswith(tuple(valid_spec)):
                    # make a copy for replace after derive
                    src_config += line
                    obj = pf.split('=')
                    if isinstance(obj, str):
                        configs.append(json.loads(obj[1]))
                    else:
                        configs.append(obj[1])
    except IOError:
        configs = []
...
if __name__ == "__main__":
    pci_passthro = {}
    sriov_nova_conf_file = '/var/lib/openstack/config/nova/03-sriov-nova.conf'
    ...
    if user_config_copy != user_configs:
        pci_passthro[_PASSTHROUGH_WHITELIST_KEY] = (user_config_copy
                                                    + derived)
        with open(pci_file_path, 'w') as pci_file:
            json.dump(pci_passthro, pci_file)

        # Replace the new derived whitelist of pci into nova
        str_replace = (_PASSTHROUGH_WHITELIST_KEY + ' = '
                       + str(pci_passthro[_PASSTHROUGH_WHITELIST_KEY]) + '\n')

        with open(sriov_nova_conf_file) as nova_config:
            file_data = nova_config.read()
            update = file_data.replace(input_config, str_replace)
